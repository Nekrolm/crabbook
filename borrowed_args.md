...
**Общее свойство всех [безопасных][^4] ссылочных типов -- у них есть параметр: время жизни (lifetime) -- область видимости, где ссылка остается валидной**
...
[^4]: Сырые указатели `*const T`, `*mut T`, обертки над ними `NonNull<T>` -- тоже являются ссылочными типами. Они свободны от вездесущих времен жизни. Однако для работы с ними уже требуется `unsafe {}`. Нет lifetime параметра -- полномочия компилятора по проверке их валидности на этом заканчиваются. 